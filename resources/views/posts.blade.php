...
<div class="relative bg-gradient-to-b from-transparent z-20 via-slate-100 via-20% to-slate-100">
    ...
    <div class="md:mx-10 mx-5 @if(!request('category') && !request('author')) mt-32 @else mt-16 @endif">
        @if (!$blogs)
            <div class="mt-2">
        ...
        <div class="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            @foreach ($posts as $post)
                <a href="/post/{{ $post->slug }}" class="relative card-hover rounded-2xl overflow-hidden sm:bg-red-200 md:min-h-[50vh] shadow-lg shadow-slate-300">
